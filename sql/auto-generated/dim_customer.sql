/**********************************************************************
artefact name :- dim_customer
description   :- dim_customer sql generated from mapping
----------------------------------------------------------------------
change log
version :   date :        description :                       changed by
----------------------------------------------------------------------
0.0         2026-08-24    auto-generated from mapping         ai_agent
**********************************************************************/

with po_dedup as (
    select * 
    from po
    where row_number() over (partition by po_id order by updated_at desc) = 1
)
select 
...
from po_dedup po
left join ad_mstr on upper(po.ad_addr) = upper(ad_mstr.ad_addr) and upper(po.ad_domain) = upper(ad_mstr.ad_domain)
left join cm_mstr on upper(cm_mstr.cm_addr) = upper(ad_mstr.ad_addr) and upper(cm_mstr.cm_domain) = upper(ad_mstr.ad_domain)
left join currency on currency.currencycode = cm_mstr.cm_curr
left join d_cntry_hier on upper(ad_mstr.ad_ctry) = upper(d_cntry_hier.cntry_alpha2_cd)
;